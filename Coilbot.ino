...
int revolution_required = 100; // variable to store the number of revolution required in the spool
...
void setup() {
  ...
  myServo.attach(servo_pin, servo_pwm_min, servo_pwm_max);  // attaches the servo on the pin to the servo object
...
void loop() {
  ...
  Serial.print(revolution_count); // display information
  Serial.print("\tRevolution left:\t"); // display information
  Serial.print(revolution_required - revolution_count); // display information
  Serial.print("\tPosition:\t"); // debugging
  ...
  if (revolution_count >= revolution_required){ // check whether the required number of revolution of wire has been spun onto the spool
    ...
    Serial.println(revolution_count); // display feedback for completion
  ...
  // check whether servo reaches end points or not and change direction accordingly
  ...
  else if (pos >= final_deg){ // if position of the servo is equal or more than final position
...
// This method simply just turn off the motors, called when ever we don't need the motors anymore.
...
// This method allows user to set initial position, final position and start position of the servo.
void calibration() {
